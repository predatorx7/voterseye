...
String extractPublicKey(String privateKeyHex) {
  ...
  String uncompressedPublicKey =
      xCoordinate.toRadixString(16).padLeft(64, '0') +
          yCoordinate.toRadixString(16).padLeft(64, '0');
...
String deriveEthereumAddress(String publicKeyHex) {
  ...
  final ethereumAddress =
      keccakHash.sublist(keccakHash.length - 20).map((byte) {
...
Uint8List deriveSymmetricKey(String privateKey) {
  ...
  final sha256Digest =
      crypto.sha256.convert(Uint8List.fromList(privateKeyBytes));
